import { getProviderUsage } from './usage';
...
function assertRequestAllowed(signal?: AbortSignal): void {
  ...
  if (maxCostUsd !== undefined && getProviderUsage().estimatedCostUsd >= maxCostUsd) {
...
export async function waitForProviderRequestSlot(signal?: AbortSignal): Promise<void> {
